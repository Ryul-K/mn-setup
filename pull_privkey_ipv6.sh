...
function POPC_privkey_ipv6() {
   tmpIPv4=$(curl -s4 icanhazip.com)

   mn_key[0]=$(sed -n '/masternodeprivkey/p' $POPC_CONFIGFOLDER/$POPC_CONFIG_FILE)
   echo " "
   echo -e "${RED} ======== POPC PRIVATEKEY & IPV6  ========${NC}"
   echo -e "${GREEN}`hostname` ${tmpIPv4} ${mn_key[0]:18:70}\t${NC}"

   for (( i = 1; i <= $SET_NUM; i++)); do
     #mn_key[$i]=$(sed -n '/masternodeprivkey/p' $CONFIGFOLDER$i/$CONFIG_FILE
     mn_key[$i]=$(sed -n '/masternodeprivkey/p' $POPC_CONFIGFOLDER$i/$POPC_CONFIG_FILE)
     echo -e "${GREEN}`hostname`-$i ${mn_IPv6[$i]} ${mn_key[$i]:18:70}\t${NC}"
   done
   echo " "
   echo " "
